...
local extract_archive_fns = {
    zip = function(src, dst, options)
        local ok, err = zip.safe_extract(src, dst, options)
        fs.remove(src)
        ami_assert(ok, "Failed to extract: " .. tostring(err))
    end,
    ["tar"] = function(src, dst, options)
        local ok, err = tar.safe_extract(src, dst, options)
        fs.remove(src)
        ami_assert(ok, "Failed to extract: " .. tostring(err))
    end,
    ["tar.gz"] = function(src, dst, options)
        local tmp_file = os.tmpname()
        local ok, err = lz.safe_extract(src, tmp_file)
        fs.remove(src)
        if not ok then
            fs.remove(tmp_file)
        ...
        local ok, err = tar.safe_extract(tmp_file, dst, options)
        fs.remove(tmp_file)
        ami_assert(ok, "Failed to extract: " .. tostring(err))
    end
}
...
local ok, files = fs.safe_read_dir("bin", { return_full_paths = true }) --[[@as DirEntry]]
ami_assert(ok, "Failed to enumerate binaries", EXIT_APP_IO_ERROR)

for _, file in ipairs(files) do
    if fs.file_type(file) == 'file' then
        local ok, err = fs.safe_chmod(file, "rwxrwxrwx")
        if not ok then
